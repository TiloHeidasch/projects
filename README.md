...
| `mosquitto` | No reliable healthcheck when auth is enabled |
...
## OpenHands GitHub webhook automations

The current OpenHands stack injects `AUTOMATION_WEBHOOK_SECRET` into the
container. Its Cloudflare Tunnel ingress is token-only, externally managed,
and not version-controlled.

Configure a dedicated public hostname and edge ingress that allows only
`POST /api/automation/v1/events/*/github` to the private Docker upstream
`http://openhands:8000`. Return `403` or `404` for every other path and
method. Do not enable Cloudflare Access or other interactive authentication on
this endpoint. Do not expose the OpenHands UI, any other generic `/api/*`
route, automation CRUD/dispatch/KV routes, OpenAPI documentation, or VS Code
routes. Forward the unmodified raw request body and these GitHub headers:
`X-Hub-Signature-256`, `X-GitHub-Event`, `X-GitHub-Delivery`, and
`Content-Type`. Set the request-body limit to 1 MiB.

Use this exact webhook URL pattern:

```text
https://<public-webhook-host>/api/automation/v1/events/<OPENHANDS_ORG_ID>/github
```

`OPENHANDS_ORG_ID` is the existing deployment-specific OpenHands organization
UUID. Obtain it only from the running deployment or platform configuration;
never generate a new value.

In GitHub, set the webhook content type to `application/json`, use the same
secret as `AUTOMATION_WEBHOOK_SECRET`, enable SSL verification, and select only
**Issues**, **Pull requests**, and **Pull request reviews** (optionally **Issue
comments**). Never select **Send me everything**.

After deployment, verify the automation-service capabilities locally:

```bash
curl -H "X-Session-API-Key: $OPENHANDS_AUTOMATION_API_KEY" \
  http://127.0.0.1:8000/api/automation/v1/capabilities
```

A successful enablement must show:
...
`AUTOMATION_BASE_URL` is not included because this repository has no
supported setting for it.
