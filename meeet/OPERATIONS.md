...
The stack contains the one-shot `compiler`, `meeet`, and `cloudflared`. Meeet
is accessed only through the cloudflared Tunnel; no host port is exposed.
Cloudflared uses Docker DNS to reach `http://meeet:3000`. Meeet starts only
...
decision path before `meeet` starts; `meeet` waits for it with
